...
if [ -n "$ENV_WARNINGS" ]; then
    echo "" >&2
    echo -e "╭─────────────────────────────────────────────────────────────╮" >&2
    echo -e "│  \033[38;5;208m⚠  Running with limited functionality\033[0m                      │" >&2
    echo -e "├─────────────────────────────────────────────────────────────┤" >&2
    echo -e "│                                                             │" >&2
    while IFS= read -r line; do
        printf "│  \033[38;5;208m%-58s\033[0m│\n" "$line" >&2
    done <<< "$ENV_WARNINGS"
    echo -e "│                                                             │" >&2
    echo -e "│  \033[2mTicket titles, statuses, and assignees will not appear.\033[0m   │" >&2
    echo -e "│  \033[2mSee .env.example for setup instructions.\033[0m                  │" >&2
    echo -e "│                                                             │" >&2
    echo -e "╰─────────────────────────────────────────────────────────────╯" >&2
    echo "" >&2
...
# Resolve current branch, even during rebase/bisect (detached HEAD).
# Falls back to rebase metadata, then worktree directory name.
resolve_current_branch() {
    ...
    # Detached HEAD — check rebase metadata
    local git_dir
    ...
    # Fall back to worktree directory name
    local toplevel
    toplevel=$(git rev-parse --show-toplevel 2>/dev/null)
    if [ -n "$toplevel" ]; then
        local wt_basename="${toplevel##*/}"
        if [[ "$wt_basename" =~ \.([^.]+)$ ]]; then
            echo "${BASH_REMATCH[1]}"
            return
        fi
...
TIME_MAX_LENGTH=26  # Width for the checked/updated column

# Every column is padded to a constant, so a rendered row is always exactly this
# wide: the six columns, the " │ " between them, and the trailing " │".
ROW_VISUAL_WIDTH=$((BRANCH_MAX_LENGTH + TITLE_MAX_LENGTH + STATUS_MAX_LENGTH \
                    + ASSIGNEE_MAX_LENGTH + TIME_MAX_LENGTH + COMMIT_MAX_LENGTH + 17))
# fzf will not match a field it does not display ("fzf doesn't allow searching
# against the hidden fields" — fzf(1), --nth), so the full-text search field
# (build_search_key) cannot simply be hidden. It is parked out here instead, past the
# right edge of the window, where fzf reads it but the terminal never draws it —
# with --no-hscroll so fzf can't scroll over to a match inside it, and --ellipsis= so
# the overflowing line doesn't earn an overflow marker. 400 clears any ordinary
# terminal, and the real width is measured so an unusually wide one doesn't expose it.
SEARCH_KEY_COLUMN=400
_rr_term_cols=$(stty size </dev/tty 2>/dev/null | cut -d' ' -f2)
[[ "$_rr_term_cols" =~ ^[0-9]+$ ]] && (( _rr_term_cols + 40 > SEARCH_KEY_COLUMN )) \
    && SEARCH_KEY_COLUMN=$((_rr_term_cols + 40))
unset _rr_term_cols
# The blank run that does the parking. Built once — it is the same for every row, and
# 200-odd spaces are not worth re-expanding for each of them.
printf -v SEARCH_KEY_INDENT '%*s' $((SEARCH_KEY_COLUMN - ROW_VISUAL_WIDTH)) ''

# Build worktree map: branch -> worktree path
declare -A WORKTREE_MAP
declare -A WORKTREE_BRANCH=()  # Reverse map: worktree path -> actual checked-out branch (authoritative)
declare -a DISPLACED_BRANCHES=()  # Branches displaced from their worktree by a mismatch (e.g., oneshot checked out a hotfix)
build_worktree_map() {
...
copy_worktree_files() {
    ...
    for file in $files_to_copy_str; do
        ...
        if [ -e "$src" ] && [ ! -e "$dst" ]; then
            ...
            if [ -n "$WORKTREE_COPY_EXCLUDE" ] && [ -d "$src" ]; then
                ...
                rsync -a $exclude_args "$src/" "$dst/" 2>/dev/null && ((copied_count++)) && total_bytes=$((total_bytes + ${src_size:-0}))
            else
                cp -r "$src" "$dst" 2>/dev/null && ((copied_count++)) && total_bytes=$((total_bytes + ${src_size:-0}))
            fi
        fi
    done
    ...
    if [ $copied_count -gt 0 ]; then
        ...
        if [ $total_bytes -ge 1073741824 ]; then
            size_display="$(awk "BEGIN {printf \"%.1f\", $total_bytes/1073741824}")GB"
        elif [ $total_bytes -ge 1048576 ]; then
            size_display="$(awk "BEGIN {printf \"%.1f\", $total_bytes/1048576}")MB"
        elif [ $total_bytes -ge 1024 ]; then
            size_display="$(awk "BEGIN {printf \"%.0f\", $total_bytes/1024}")KB"
        else
            size_display="${total_bytes}B"
        fi
        echo "✓ Copied $copied_count file(s) to worktree ($size_display)" >&2
    fi
...
# Create a branch + worktree for a ticket that doesn't have a local branch yet.
# If the ticket's branch exists on the remote, tracks it; otherwise creates from main.
# Prints the worktree path to stdout on success.
...
create_ticket_worktree() {
    ...
    if ! git show-ref --verify --quiet refs/heads/main; then
        if git show-ref --verify --quiet refs/heads/master; then
            ...
            echo "✗ Cannot find main/master branch to base new branch on" >&2
            return 1
        fi
    fi

    # Build worktree name with JIRA title if available
    load_jira_caches
    local repo_name=$(basename "$GIT_ROOT")
    local jira_title=$(get_jira_title "$ticket_id")
    local wt_name="$ticket_id"
    ...
    if git show-ref --verify --quiet "refs/remotes/origin/$ticket_id"; then
...
# Build the off-screen search field appended to every row.
#
# The visible columns are truncated to fit (branch at BRANCH_MAX_LENGTH, title at
# TITLE_MAX_LENGTH, status at STATUS_MAX_LENGTH, assignee at ASSIGNEE_MAX_LENGTH), and
# fzf can only match text it puts on screen — so "UB-1500-perf-s..." was a branch you
# could see and could not find, and "IN QUALIF..." a status you could not filter by.
# This field carries all four in full, indented past SEARCH_KEY_COLUMN so it is part of
# the line fzf searches without being part of the line the terminal draws.
#
# Args: full_branch (may carry a TICKET:/REMOTE: prefix), title, status, assignee
# Sets: SEARCH_KEY (a global, not stdout — this runs once per row in the streaming
#       render loop, where a $() fork per row is measurable)
build_search_key() {
    local _branch="$1" _title="$2" _status="$3" _assignee="$4"
    _branch="${_branch#TICKET:}"
    _branch="${_branch#REMOTE:}"
    [ "$_title" = "<EMPTY>" ] && _title=""
    [ "$_status" = "<EMPTY>" ] && _status=""
    [ "$_assignee" = "<UNASSIGNED>" ] && _assignee=""
    SEARCH_KEY="$_branch $_title $_status $_assignee"
    # │ is the field delimiter — a title containing one would split this field in two
    SEARCH_KEY="${SEARCH_KEY//│/ }"
    SEARCH_KEY="${SEARCH_KEY_INDENT}${SEARCH_KEY}"
}
...
show_help() {
    ...
    echo -e "\033[38;5;141m┃\033[0m                    \033[1mRR — Help\033[0m                    \033[38;5;141m┃\033[0m"
    echo -e "\033[38;5;141m┗━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━┛\033[0m"
    echo ""
    ...
    echo -e "  \033[33mF8\033[0m           Delete worktree / branch (interactive)"
    echo ""
    ...
    echo -e "\033[1;36mROW TYPES\033[0m  \033[2m(example rows)\033[0m"
    echo ""
    echo -e "  \033[38;5;141m★ UB-1234     \033[0m \033[38;5;250m⊙\033[0m   │ \033[38;5;141myour ticket, has worktree             \033[0m │ \033[38;5;221m● IN PROGRESS \033[0m │ \033[38;5;109myou            \033[0m │"
    echo -e "  \033[38;5;141m★ UB-1234     \033[0m \033[38;5;250m⊙\033[38;5;214m !\033[0m │ \033[38;5;141myour ticket, worktree is dirty        \033[0m │ \033[38;5;81m⬡ MR          \033[0m │ \033[38;5;109myou            \033[0m │"
    echo -e "  \033[38;5;244m· \033[38;5;103mUB-1234-wip \033[0m \033[38;5;250m⊙\033[0m   │ \033[38;5;103myours, alternate branch name          \033[0m │ \033[38;5;81m⬡ MR          \033[0m │ \033[38;5;244myou            \033[0m │"
    echo -e "    \033[38;5;250mUB-5678         \033[0m │ \033[38;5;109mnot yours, no worktree                \033[0m │ \033[38;5;109m◇ QA          \033[0m │ \033[38;5;244msomeone        \033[0m │"
    echo -e "  \033[38;5;71m+ UB-9999         \033[0m │ \033[38;5;71mJIRA ticket, no branch yet            \033[0m │ \033[38;5;250m○ TO DO       \033[0m │ \033[38;5;244myou            \033[0m │ \033[2m← Enter creates branch\033[0m"
    echo -e "  \033[2m+ UB-8888         \033[0m │ \033[2mabandoned ticket, at bottom           \033[0m │ \033[2;38;5;244m✓ ABANDONED   \033[0m │ \033[2myou            \033[0m │"
    echo -e "  \033[38;5;67m↑ UB-4567         \033[0m │ \033[38;5;67mexists on origin, not local           \033[0m │ \033[38;5;221m● IN PROGRESS \033[0m │ \033[38;5;244msomeone        \033[0m │"
    if [ "$RR_PANE_MGMT_ENABLED" = "true" ] && [ "$PANE_COUNT" -gt 0 ]; then
        echo ""
        echo -e "  \033[2mPane indicators appear after ⊙ when a pane is in that worktree:\033[0m"
        echo -n "  "
        for i in "${!PANE_IDS[@]}"; do
...
generate_instant_data() {
    ...
    _current_branch=$(resolve_current_branch)
    ...
    # instead of ~1300ms.
    # Use WORKTREE_BRANCH (populated by build_worktree_map) as the authoritative
    # source for which branch is checked out in each worktree. This avoids the
    # iteration-order problem when WORKTREE_MAP has both eponymous and actual
    # branch keys for the same path (e.g., both "UB-6723" and
    # "UB-6723-duplicate-charts-per-direction-passenger" when the worktree dir
    # includes a JIRA title suffix).
    ...
    for _wt_branch in "${!WORKTREE_MAP[@]}"; do
        ...
        # When the worktree dir includes extra info (e.g., JIRA title like
        # repo.UB-6668-implement-foo), the eponymous name won't match the real
        # branch. Use the actual branch (from WORKTREE_BRANCH) for display/field7.
        local _display_branch="$_epo_branch"
        local _actual_branch="${WORKTREE_BRANCH[$_wt_path]:-}"
        if [ -n "$_actual_branch" ] && [ "$_actual_branch" != "$_epo_branch" ]; then
            _display_branch="$_actual_branch"
            # If this is a genuine mismatch (not just a title-suffixed dir name),
            # track the displaced branch so it appears in the branch list.
            if [[ "$_epo_branch" != "$_display_branch"* ]]; then
                local _displaced_ticket=""
                [[ "$_epo_branch" =~ (${JIRA_PROJECT_REGEX}-[0-9]+) ]] && _displaced_ticket="${BASH_REMATCH[1]^^}"
                if [ -n "$_displaced_ticket" ] && [ -n "${VALID_BRANCH_REFS[$_displaced_ticket]+x}" ]; then
                    DISPLACED_BRANCHES+=("$_displaced_ticket")
                fi
            fi
        fi
        ...
        local _ticket=""
        [[ "$_display_branch" =~ (${JIRA_PROJECT_REGEX}-[0-9]+) ]] && _ticket="${BASH_REMATCH[1]^^}"
        ...
        local _truncated="$_display_branch"
        ...
        printf -v _row "%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s" \
            ...
            "checked:$_ts" " " \
            "$_display_branch" "$_jira_assignee" "WT" "$_wt_path" " "

        _instant_rows+="${_ts}"$'\t'"${_row}"$'\n'

        # Claim both display branch and eponymous branch to prevent duplicates
        if [ -n "$claimed_file" ]; then
            echo "$_display_branch" >> "$claimed_file"
            [ "$_epo_branch" != "$_display_branch" ] && echo "$_epo_branch" >> "$claimed_file"
        fi
...
generate_worktree_data() {
    ...
    _emit_worktree_data_row() {
        ...
        # When the actual branch differs from the eponymous name (e.g., worktree dir
        # includes JIRA title like repo.UB-6668-implement-foo but branch is UB-6668),
        # use the actual branch for display and field7 so switching works correctly.
        local display_branch="$eponymous_branch"
        if [ -n "$actual_branch" ] && [ "$actual_branch" != "$eponymous_branch" ]; then
            display_branch="$actual_branch"
            # Only show mismatch indicator if the actual branch is genuinely different
            # (not just a title-suffixed directory name like repo.UB-6668-fix-widget)
            if [[ "$eponymous_branch" != "$actual_branch"* ]]; then
        ...
        truncated_branch=$(truncate "$display_branch" $BRANCH_MAX_LENGTH)
        ...
        if [ -n "$claimed_file" ]; then
            ...
            if [ -n "$actual_branch" ] && [ "$actual_branch" != "$eponymous_branch" ]; then
                echo "$actual_branch" >> "$claimed_file"
            fi
        fi
    }
...
generate_branch_data() {
    ...
    if [ "$SORT_BY_COMMIT" = true ]; then
        ...
        # Also ensure displaced branches are included (branches that should be in a worktree
        # but another branch is checked out there, e.g., oneshot left a hotfix behind)
        local displaced_additions=""
        for displaced_branch in "${DISPLACED_BRANCHES[@]}"; do
            if ! echo "$branch_list" | grep -q "^$displaced_branch	"; then
                local d_unix_time
                d_unix_time=$(git log -1 --pretty=format:'%ct' "refs/heads/$displaced_branch" 2>/dev/null || echo "0")
                local d_time
                d_time=$(git log -1 --pretty=format:'%cr' "refs/heads/$displaced_branch" 2>/dev/null || echo "unknown")
                displaced_additions="${displaced_additions}${displaced_branch}	${d_unix_time}	${d_time}"$'\n'
            fi
        done
        if [ -n "$displaced_additions" ]; then
            branch_list=$(echo -e "$branch_list\n$displaced_additions" | grep -v '^$')
        fi
    fi
...
get_jira_title() {
    local ticket=$1
    if [[ $ticket =~ ^${JIRA_PROJECT_REGEX}-[0-9]+$ ]]; then
...
generate_branchless_ticket_data() {
    ...
    _emit_branchless_row() {
        ...
        [ -z "$ticket" ] && return
        [[ ! "$ticket" =~ ^${JIRA_PROJECT_REGEX}-[0-9]+$ ]] && return
    ...
    # Helper to check if a status is abandoned/terminal
    _is_abandoned_status() {
        local s="${1,,}"
        [[ "$s" == *abandon* || "$s" == *cancelled* || "$s" == *"won't do"* || "$s" == *wontdo* ]]
    }

    local _deferred_abandoned=""
    ...
    if [ -n "$assigned" ]; then
        while IFS=$'\t' read -r ticket title status assignee updated_ts; do
            ...
            if _is_abandoned_status "$status"; then
                _deferred_abandoned+="${ticket}"$'\t'"${title}"$'\t'"${status}"$'\t'"${assignee}"$'\t'"${updated_ts}"$'\n'
            else
    ...
    # Section 3: abandoned tickets at the very end
    if [ -n "$_deferred_abandoned" ]; then
        while IFS=$'\t' read -r ticket title status assignee updated_ts; do
            [ -z "$ticket" ] && continue
            _emit_branchless_row "$ticket" "$title" "$status" "$assignee" "$updated_ts"
        done <<< "$_deferred_abandoned"
    fi
}
...
format_status() {
    ...
    case "$status_lower" in
        *"abandon"*|*"cancelled"*|*"won't do"*|*"wontdo"*)
            # Dim gray - abandoned/cancelled
            icon="✓" color="2;38;5;244" text="$status_upper"
            ;;
...
get_current_state() {
    local current_branch=$(resolve_current_branch)
...
format_current_branch_as_row() {
    ...
    local time_info="$(printf "%-${TIME_MAX_LENGTH}s" "")"
...
if [ "$GENERATE_MORE_MODE" = true ]; then
    ...
    while IFS=$'\t' read -r branch title status author time_info commit_info full_branch assignee wt_indicator wt_path wt_status; do
        ...
        # Off-screen search field: what the columns had to truncate, in full
        build_search_key "$full_branch" "$title" "$status" "$assignee"
...
selected_line=$(
    {
        ...
        eval "fzf --ansi \
            ...
            --bind 'ctrl-o:execute-silent(echo OPEN_CURSOR > ~/.cache/rr/cursor_flag)+accept' \
            --delimiter='│' \
            --with-nth=1,2,3,4,5,6,9 \
            --nth=1,2,3,4,7 \
            --no-hscroll \
            --ellipsis= \
...
if [ -f "$ACTION_FILE" ]; then
    ...
    if [[ "$action_line" =~ ^CREATE_WT: ]]; then
        ...
        if [[ "$branch" == TICKET:* ]]; then
            ...
            wt_path=$(create_ticket_worktree "$ticket_id") || exit 1
            navigate_to_worktree "$wt_path"
            exit 0
        fi
        ...
        if [ -n "$branch" ]; then
            ...
            if [ -n "$existing_wt" ]; then
                ...
                record_worktree_access "$existing_wt"
        ...
        if [ -n "$branch" ]; then
            ...
            # Check merge status against main
            default_branch="main"
            ...
            if git merge-base --is-ancestor "$branch" "origin/$default_branch" 2>/dev/null; then
                ...
                unmerged_count=$(git rev-list --count "origin/$default_branch..$branch" 2>/dev/null || echo "?")
            fi

            # Show merge status
            echo ""
            if [ "$merged" = "yes" ]; then
                echo -e "  \033[32m✓ Fully merged\033[0m into $default_branch"
            else
                echo -e "  \033[33m⚠ UNMERGED\033[0m — $unmerged_count commit(s) ahead of $default_branch"
            fi
            echo ""
            ...
            if [ -n "$wt_path" ] && [ "$wt_path" != "$GIT_ROOT" ]; then
                opts+=("● Remove worktree only (keep branch)")
                opts+=("● Remove worktree + delete local branch")
            else
                opts+=("● Delete local branch")
            fi
            opts+=("○ Cancel")

            wt_label=""
            [ -n "$wt_path" ] && wt_label=" │ wt: $(basename "$wt_path")"
            merge_label=""
            if [ "$merged" = "yes" ]; then
                merge_label=" │ ✓ merged"
            else
                merge_label=" │ ⚠ $unmerged_count unmerged"
            fi
            choice=$(printf '%s\n' "${opts[@]}" | fzf --ansi --height=$((${#opts[@]} + 3)) --reverse --header="Delete: $branch$wt_label$merge_label" --header-first 2>/dev/tty)

            if [ -z "$choice" ] || echo "$choice" | grep -q "Cancel"; then
                exit 0
            fi

            # If we're currently inside the worktree being removed, cd to main repo
            current_dir=$(pwd -P 2>/dev/null || pwd)
            if [ -n "$wt_path" ] && [[ "$current_dir" == "$wt_path"* ]]; then
                navigate_to_worktree "$GIT_ROOT"
            fi

            # Remove worktree if it exists and option selected
            if echo "$choice" | grep -q "worktree"; then
                if [ -n "$wt_path" ] && [ "$wt_path" != "$GIT_ROOT" ]; then
            ...
            # Delete local branch if option selected
            if echo "$choice" | grep -q "local" || echo "$choice" | grep -q "delete local"; then
...
if [ -n "$branch" ]; then
    ...
    if [ -n "$wt_path" ]; then
        # Check if the worktree has a different branch checked out (mismatch)
        ...
        if [ -n "$wt_actual_branch" ] && [ "$wt_actual_branch" != "$branch" ] && [ -z "$mismatch_skip" ]; then
            ...
            echo "" >&2
            echo -e "  \033[38;5;208m⚠  Worktree mismatch\033[0m" >&2
            echo -e "  \033[2mWorktree\033[0m  $wt_basename" >&2
            echo -e "  \033[2mExpected\033[0m  \033[38;5;141m$branch\033[0m" >&2
            echo -e "  \033[2mActual\033[0m    \033[38;5;167m$wt_actual_branch\033[0m" >&2
            echo "" >&2

            opt1=$(echo -e "\033[38;5;114m● Switch worktree back to '$branch'\033[0m")
            opt2=$(echo -e "\033[38;5;244m○ Go to worktree as-is (on '$wt_actual_branch')\033[0m")

            choice=$(printf '%s\n%s\n' "$opt1" "$opt2" | fzf --ansi --height=6 --reverse --header="Worktree '$wt_basename' has '$wt_actual_branch' checked out instead of '$branch'" --header-first 2>/dev/tty)
            ...
            case "$choice" in
                *"Switch worktree back"*)
                    ...
                    ;;
                *"Go to worktree as-is"*)
                    # Just navigate there without switching
                    ;;
                *)
                    exit 0
                    ;;
            ...
            # Restore original stdout so RR_CD: output is captured by the shell function
            exec 1>&${_rr_saved_stdout} {_rr_saved_stdout}>&-
        fi
        ...
        if [ "$is_in_worktree" = true ] && [ "$branch" = "$expected_branch" ]; then
            ...
            # 'main' always goes directly to main repo without prompting (just cd, no branch switch)
            if [ "$branch" = "main" ] || [ "$branch" = "master" ]; then
                record_worktree_access "$GIT_ROOT"
                echo "RR_CD:$GIT_ROOT"
                exit 0
            else
            ...
            case "$choice_num" in
                1)
                    ...
                    if git worktree add "$new_wt_path" "$branch" 2>&1; then
                        ...
                        record_worktree_access "$new_wt_path"
            ...
            # In main repo - offer to create worktree or switch in-place
            current_branch=$(git rev-parse --abbrev-ref HEAD 2>/dev/null)

            # 'main'/'master' selected from main repo - just switch to it
            if [ "$branch" = "main" ] || [ "$branch" = "master" ]; then
                ...
                if ! git diff-index --quiet HEAD 2>/dev/null; then
                    echo "" >&2
                    echo "⚠ Main repo is on '$current_branch' (expected '$branch') with uncommitted changes" >&2
                    echo "" >&2
                    echo "  Fix: commit or stash first:" >&2
                    echo "       git add . && git commit -m 'wip'  # or: git stash" >&2
                    echo "" >&2
                    exit 1
                fi
                echo "Switching main repo from '$current_branch' to '$branch'..." >&2
                smart_git_switch "$branch"
                [ $? -eq 2 ] && exit 0
                exit 0
            fi

            # Option 1: Create worktree (recommended)
            opt1=$(echo -e "\033[38;5;114m● Create worktree for '$branch' and go there\033[0m")
            # Option 2: Switch in main repo
            opt2=$(echo -e "\033[2;38;5;244m○ Switch main repo to '$branch' (no worktree)\033[0m")

            choice=$(printf '%s\n%s\n' "$opt1" "$opt2" | fzf --ansi --height=6 --reverse --header="Branch '$branch' has no worktree" --header-first 2>/dev/tty)
            ...
            elif echo "$choice" | grep -q "Switch main repo"; then
                # Original behavior - switch in main repo
                if [ "$is_remote_selection" = true ] && ! git show-ref --verify --quiet "refs/heads/$branch"; then
                    echo "Creating local branch '$branch' tracking 'origin/$branch'..." >&2
                    smart_git_switch "$branch" "-c" "origin/$branch"
                    [ $? -eq 2 ] && exit 0
