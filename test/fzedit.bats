...
# tig -> fzedit bridge
#
# tig derives %(file) by parsing the diff text it renders. With
# `set diff-highlight = "delta"` that text no longer has the `--- a/…` / `+++ b/…`
# lines it looks for, so %(file) is empty and anything built on it silently does
# nothing. Measured on the same diff line: plain gives file=[src/app.ts] lineno=[0],
# delta gives file=[] lineno=[17] (a view line, not a file line).
# --------------------------------------------------------------------------

setup_bridge() {
    BRIDGE="$FZEDIT_REPO_ROOT/bin/fzedit-from-tig"
    printf '#!/bin/bash\nprintf "%%s\\n" "$*"\n' > "$TEST_TMPDIR/fakebin/fzedit-stub"
    printf '#!/bin/bash\nhead -1\n' > "$TEST_TMPDIR/fakebin/fzf-first"
    chmod +x "$TEST_TMPDIR/fakebin/fzedit-stub" "$TEST_TMPDIR/fakebin/fzf-first"
    cd "$MAIN"
    echo two > src/second.ts
    git_q "$MAIN" add -A
    git_q "$MAIN" commit -q -m "touches two files"
    SHA="$(git -C "$MAIN" rev-parse HEAD)"
}

bridge() { FZEDIT_BIN="$TEST_TMPDIR/fakebin/fzedit-stub" FZEDIT_FZF="$TEST_TMPDIR/fakebin/fzf-first" "$BRIDGE" "$@"; }

@test "bridge: with %(file) it opens that file at that line" {
    setup_bridge
    run bridge "src/app.ts" "42" "$SHA"
    [ "$output" = "--tab src/app.ts 42" ]
}

@test "bridge: with delta's empty %(file) it still opens something" {
    setup_bridge
    run bridge "" "17" "$SHA"
    [[ "$output" == "--tab src/"* ]] || { echo "got: $output"; return 1; }
}

@test "bridge: delta's bogus lineno is not passed through" {
    # Under delta %(lineno) is a view line; jumping to it would land anywhere.
    setup_bridge
    run bridge "" "17" "$SHA"
    [[ "$output" != *" 17" ]]
}

@test "bridge: a single-file commit needs no picker" {
    setup_bridge
    echo three > "$MAIN/src/app.ts"
    git_q "$MAIN" commit -qam "one file only"
    run env FZEDIT_BIN="$TEST_TMPDIR/fakebin/fzedit-stub" FZEDIT_FZF=/bin/false \
        "$BRIDGE" "" "0" "$(git -C "$MAIN" rev-parse HEAD)"
    [ "$output" = "--tab src/app.ts" ]
}

@test "bridge: an uncommitted status row just scopes a tab" {
    setup_bridge
    run bridge "" "0" "0000000000000000000000000000000000000000"
    [ "$output" = "--tab" ]
}

@test "bridge: nothing to go on still scopes a tab rather than erroring" {
    setup_bridge
    run bridge "" "" ""
    [ "$status" -eq 0 ]
    [ "$output" = "--tab" ]
