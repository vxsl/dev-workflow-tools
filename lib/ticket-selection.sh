...
refresh_tickets_cache_if_needed() {
    # Helper: cache is usable if it exists AND has at least one issue
    local cache_has_tickets=false
    if [ -f "$TICKETS_CACHE" ]; then
        local issue_count
        issue_count=$(jq -r '.issues | length' "$TICKETS_CACHE" 2>/dev/null || echo 0)
        [ "$issue_count" -gt 0 ] 2>/dev/null && cache_has_tickets=true
    fi

    if [ -n "$TICKET_FETCH_PID" ]; then
        # If cache already has tickets, let the background fetch finish silently
        if [ "$cache_has_tickets" = true ]; then
            return 0
        fi
        # Cache is missing or empty — wait for the background fetch before opening fzf
...
select_ticket_or_slack() {
    ...
    if [[ "$query" =~ ^https://.*slack\.com/archives/([A-Z0-9]+)/p([0-9]+) ]] || \
       [[ "$query" =~ ^https://.*slack\.com/archives/([A-Z0-9]+)/?$ ]] || \
       [[ "$query" =~ ^https://.*slack\.com/client/[A-Z0-9]+/([A-Z0-9]+) ]]; then
        echo "SLACK:$query"
        return 0
    fi
